...
public class AIManager {
    ...
    public void trimConversationHistory(String model, long customId) {
        ...
        Encoding encoding;
        try {
            encoding = registry.getEncodingForModel(model);
        } catch (Exception e) {
            System.out.println("Tokenizer not available for model: " + model + ", using cl100k_base as fallback.");
            encoding = registry.getEncoding("cl100k_base"); // safe fallback for OpenAI chat models
        }
    
        int contextLimit = contextInfo.upperLimit();
        int promptTokens = countTokens(history, encoding);
    ...
    private int countTokens(List<Map<String, Object>> history, Encoding encoding) {
        int total = 0;
        ...
        return inputArray.thenCompose(messages ->
            CompletableFuture.supplyAsync(() -> {
                ...
                try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
                    ...
                    int promptTokens = messagesList.stream()
                        .mapToInt(m -> countTokens((String) m.get("content")) + countTokens((String) m.get("role")))
                        .sum();
                    int contextLimit = contextInfo.upperLimit(); // e.g., 128000
                    int outputLimit = outputInfo.upperLimit();   // e.g., 4096
                    int calculatedMaxTokens = Math.max(0, Math.min(outputLimit, contextLimit - promptTokens));
                    if (status) {
    ...
    public static CompletableFuture<String> template(long customId, CompletableFuture<List<MessageContent>> inputArray) {

        try {
            ModelInfo contextInfo = ModelRegistry.OPENAI_CHAT_COMPLETION_MODEL_OUTPUT_LIMITS.get(ConfigManager.getStringValue("openai_chat_model"));
            CompletableFuture<Map<String, String>> map = getResponse(
                customId,
                inputArray,
                contextInfo.upperLimit(),
                ConfigManager.getStringValue("openai_chat_model"),
...
//    
//        int contextLimit = contextInfo.upperLimit();
//        int promptTokens = history.stream()
//            .mapToInt(msg -> estimateTokens(String.valueOf(msg.get("content"))))
//            .sum();
//    
//        while (promptTokens > contextLimit && !history.isEmpty()) {
//            Map<String, Object> removed = history.remove(0);
//            promptTokens -= estimateTokens(String.valueOf(removed.get("content")));
//        }
//    
//        conversations.put(customId, history);
//    }
